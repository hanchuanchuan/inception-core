...
func (s *inceptionCore) ProcessChan(wg *sync.WaitGroup) {
	for {
		r := <-s.ch
...
func (s *inceptionCore) GetNextBackupRecord() *Record {
	for {
		r := s.recordSets.Next()
		if r == nil {
			return nil
...
func (s *inceptionCore) Parser(ctx context.Context) {
...
func (s *inceptionCore) checkFilter(event *replication.RowsEvent,
...
func (s *inceptionCore) checkUpdateFilter(event *replication.RowsEvent,
...
// 解析的sql写入缓存,并定期入库
func (s *inceptionCore) myWrite(b []byte, binEvent *replication.BinlogEvent,
...
// 解析的sql写入缓存,并定期入库
func (s *inceptionCore) myWriteDDL(sql string, opid string, table string, record *Record) {
...
func (s *inceptionCore) flush(table string, record *Record) {
...
func (s *inceptionCore) checkError(e error) {
	if e != nil {
		log.Error(e)
...
func (s *inceptionCore) write(b []byte, binEvent *replication.BinlogEvent) {
	// 此处执行状态不确定的记录
...
func (s *inceptionCore) generateInsertSql(t *TableInfo, e *replication.RowsEvent,
...
func (s *inceptionCore) generateDeleteSql(t *TableInfo, e *replication.RowsEvent,
	binEvent *replication.BinlogEvent) (string, error) {
	var buf []byte
	if len(t.Fields) < int(e.ColumnCount) {
		return "", errors.Errorf("表%s.%s缺少列!当前列数:%d,binlog的列数%d",
...
func (s *inceptionCore) generateUpdateSql(t *TableInfo, e *replication.RowsEvent,
	binEvent *replication.BinlogEvent) (string, error) {
	var buf []byte
	if len(t.Fields) < int(e.ColumnCount) {
		return "", errors.Errorf("表%s.%s缺少列!当前列数:%d,binlog的列数%d",
