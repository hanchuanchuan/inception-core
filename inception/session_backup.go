...
func (s *inceptionCore) ProcessChanBackup(wg *sync.WaitGroup) {
	for {
		r := <-s.chBackupRecord
...
func (s *inceptionCore) runBackup(ctx context.Context) {
...
// 解析的sql写入缓存,并定期入库
func (s *inceptionCore) writeBackupRecord(dbname string, record *Record, values []interface{}) {
...
func (s *inceptionCore) flushBackupRecord(dbname string, record *Record) {
