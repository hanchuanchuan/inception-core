...
func (s *testSessionIncTranSuite) TestInsert(c *C) {
	...
	s.mustRunBackupTran(c, "drop table if exists t1;create table t1(id int);")
	s.assertRows(c, s.rows[2:], "DROP TABLE `test_inc`.`t1`;")

	s.mustRunBackupTran(c, "insert into t1 values(1);")
	s.assertRows(c, s.rows[1:], "DELETE FROM `test_inc`.`t1` WHERE `id`=1;")
...
insert into t22(id,c1) values(6,"6");`)
	...
	// 主键冲突时
	s.runTranSQL(`drop table if exists t1;
create table t1(id int primary key,c1 varchar(100),c2 int);
insert into t1(id,c1) values(1,"1");
insert into t1(id,c1) values(2,"2");
...
func (s *testSessionIncTranSuite) insertMulti(c *C, batch int) {
	...
	s.runTranSQL(strings.Join(insert_sql, "\n"), batch)
...
func (s *testSessionIncTranSuite) TestUpdate(c *C) {
	...
	s.mustRunBackupTran(c, "update t1 set c1=10 where id = 1;")
	row := s.rows[s.getAffectedRows()-1]
	backup := s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		"UPDATE `test_inc`.`t1` SET `id`=1, `c1`=1 WHERE `id`=1 AND `c1`=10;", Commentf("%v", s.rows))

	s.mustRunBackupTran(c, `drop table if exists t1;
        create table t1(id int primary key,c1 int);
        insert into t1 values(1,1),(2,2);`)
	s.mustRunBackupTran(c, "update t1 set id=id+2 where id > 0;")
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		strings.Join([]string{
...
func (s *testSessionIncTranSuite) TestMinimalUpdate(c *C) {
	...
	row := s.rows[s.getAffectedRows()-1]
	backup := s.query("t1", row[7].(string))
	c.Assert(backup, Equals, "UPDATE `test_inc`.`t1` SET `c1`=1 WHERE `id`=1 AND `c1`=10;", Commentf("%v", s.rows))

	s.mustRunBackupTran(c, `drop table if exists t1;
        create table t1(id int primary key,c1 int);
        insert into t1 values(1,1),(2,2);`)

	s.mustRunBackupTran(c, "update t1 set c1=10 where id > 0;")
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		strings.Join([]string{
	...
	s.mustRunBackupTran(c, "update t1 set c1=2 where id > 0;")
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		"UPDATE `test_inc`.`t1` SET `c1`=1 WHERE `id`=1;", Commentf("%v", s.rows))

	s.mustRunBackupTran(c, `drop table if exists t1;
        create table t1(id int primary key,c1 tinyint unsigned,c2 varchar(100));
        insert into t1 values(1,127,'t1'),(2,130,'t2');`)

	s.mustRunBackupTran(c, "update t1 set c1=130,c2='aa' where id > 0;")
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		strings.Join([]string{
...
func (s *testSessionIncTranSuite) TestDelete(c *C) {
	...
	s.mustRunBackupTran(c, "delete from t1 where id <= 2;")
	row := s.rows[s.getAffectedRows()-1]
	backup := s.query("t1", row[7].(string))
	c.Assert(backup, Equals,
		strings.Join([]string{
	...
	s.mustRunBackupTran(c, "delete from t1;")
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
    ...
    insert into t1 values(1,'{"time":"2015-01-01 13:00:00","result":"fail"}');`)
		...
		row = s.rows[s.getAffectedRows()-1]
		backup = s.query("t1", row[7].(string))
		c.Assert(backup, Equals, "INSERT INTO `test_inc`.`t1`(`id`,`c1`) VALUES(1,'{\\\"result\\\":\\\"fail\\\",\\\"time\\\":\\\"2015-01-01 13:00:00\\\"}');", Commentf("%v", s.rows))
	...
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals, "INSERT INTO `test_inc`.`t1`(`id`,`c1`) VALUES(1,2);", Commentf("%v", s.rows))
	...
	row = s.rows[s.getAffectedRows()-1]
	backup = s.query("t1", row[7].(string))
	c.Assert(backup, Equals, "INSERT INTO `test_inc`.`t1`(`id`,`c1`) VALUES(1,1.11);", Commentf("%v", s.rows))
...
func (s *testSessionIncTranSuite) TestCreateTable(c *C) {
	...
	s.mustRunBackupTran(c, `DROP TABLE IF EXISTS t1,t2;
