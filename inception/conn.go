...
func (s *inceptionCore) createNewConnection(dbName string) {
...
func (s *inceptionCore) Raw(sqlStr string) (rows *sql.Rows, err error) {
	// 连接断开无效时,自动重试
...
// Raw 执行sql语句,连接失败时自动重连,自动重置当前数据库
func (s *inceptionCore) Exec(sqlStr string, retry bool) (res sql.Result, err error) {
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
// Raw 执行sql语句,连接失败时自动重连,自动重置当前数据库
func (s *inceptionCore) RawScan(sqlStr string, dest interface{}) (err error) {
	// 连接断开无效时,自动重试
	for i := 0; i < maxBadConnRetries; i++ {
...
func (s *inceptionCore) initConnection() (err error) {
	name := s.DBName
	if name == "" {
		name = s.opt.db
	}
